...
export default function EditClient() {
  ...
  const epParam  = params.episode as string;
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error: err }) => {
        ...
        const univs = data.universes as Universe[];
        setUniverses(univs);
        const mainU = univs.find((u) => u.isMain) ?? univs[0];
        const ep    = mainU?.episodes[epIndex];
        if (!ep) { setError("에피소드를 찾을 수 없습니다."); setLoading(false); return; }
        setTitle(ep.title);
        setContent(ep.content);
        setCoverImageUrl(ep.coverImageUrl ?? "");
        setLoading(false);
      });
  }, [storyId, epIndex, authLoading]);
  ...
  const handleSave = async () => {
    if (!universes || saving) return;
    setSaving(true);
    const updated = universes.map((u) => {
      if (!u.isMain) return u;
      return {
        ...u,
        episodes: u.episodes.map((ep, i) =>
          i === epIndex
            ? { ...ep, title, content, coverImageUrl: coverImageUrl || undefined }
            : ep
        ),
      };
    });
    ...
    router.push(`/reader/${storyId}/${epIndex}`);
  };

  const handleCancel = () => router.push(`/reader/${storyId}/${epIndex}`);
  ...
  return (
    <div className="w-full min-h-screen bg-black flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 flex-shrink-0">
        ...
        <span className="text-white/40 text-xs">{epIndex + 1}화 수정</span>
        <button
          onClick={handleSave}
