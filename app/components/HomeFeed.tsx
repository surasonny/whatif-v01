...
function computeBattle(story: Story): Omit<BattleInfo, "battleRank"> {
  ...
  return {
    ...
    gapToTransfer: Math.max(0, Math.round(mainLikes * TRANSFER_THRESHOLD - topLikes)),
    progressPct: mainLikes > 0 ? Math.min((topLikes / (mainLikes * TRANSFER_THRESHOLD)) * 100, 100) : 0,
  };
}
...
function sortStories(stories: Story[]): SortedStory[] {
  ...
  return [...active, ...withBattle.filter((s) => !activeIds.has(s.id))];
}
...
async function loadWeeklyChampion(stories: Story[]): Promise<string | null> {
  ...
  const { data } = await supabase
    .from("main_transfers").select("story_id").gte("transferred_at", weekStart.toISOString());
  if (!data || data.length === 0) return null;
  const counts: Record<string, number> = {};
  for (const row of data as { story_id: string }[]) counts[row.story_id] = (counts[row.story_id] ?? 0) + 1;
  const topId = Object.entries(counts).sort((a, b) => b[1] - a[1])[0]?.[0];
  return topId ? (stories.find((s) => s.id === topId)?.title ?? null) : null;
}
...
export default function HomeFeed() {
  ...
  // ── 스와이프 핸들러 (JSX에 직접 등록) ──────────────────────────
  function onSwipeStart(y: number) {
    startYRef.current = y;
  }

  function onSwipeEnd(y: number) {
    if (startYRef.current === null) return;
    const diff = startYRef.current - y;
    startYRef.current = null;
    if (diff > 50)       setCurrentIndex((p) => Math.min(p + 1, sortedStories.length - 1));
    ...
    const updatedStories = appState.stories.map((s) => {
      ...
      return {
        ...
        universes: s.universes.map((u) => {
          ...
          return { ...u, episodes: u.episodes.map((ep, ei) => ei === lastIdx ? { ...ep, likes: ep.likes + 1 } : ep) };
        }),
      };
    });
  ...
  return (
    <>
      ...
      <div
        ...
        // 포인터 스와이프 (PC 마우스/스타일러스) — touch는 onTouchStart/End에서 처리
        onPointerDown={(e) => { if (e.pointerType !== "touch") onSwipeStart(e.clientY); }}
        onPointerUp={(e) => { if (e.pointerType !== "touch") onSwipeEnd(e.clientY); }}
      >
        {/* 배경 */}
        {bgImageUrl ? (
          <img
        ...
        <div className="absolute top-0 left-0 right-0 z-10">
          ...
          {weeklyChampion && (
            <div
              className="mx-4 mb-1 px-3 py-1.5 rounded-xl flex items-center gap-2"
              style={{ backgroundColor: "rgba(251,191,36,0.12)", border: "1px solid rgba(251,191,36,0.25)", backdropFilter: "blur(8px)" }}
            >
            ...
            <div className="mb-4">
              ...
              <div className="w-full h-2 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.1)" }}>
                <div
                  ...
                  style={{ width: `${story.progressPct}%`, background: "linear-gradient(to right, #f97316, #ef4444)" }}
                />
              </div>
